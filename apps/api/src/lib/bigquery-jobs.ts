...
try {
  ...
  logger.warn(
    "BigQuery client not available. BigQuery logging will be disabled.",
    {
      error: error.message,
    },
  );
    ...
    if (exists) {
      ...
      try {
        ...
        if (
          ...
          await table.delete();
          logger.info("Deleted table with no schema", {
            dataset: datasetId,
            table: tableId,
          });
        } else {
          // Check if the table has the old schema with docs field or wrong data types
          const hasDocsField = currentSchema.fields.some(
            (field: any) => field.name === "docs",
          );
          const timeTakenField = currentSchema.fields.find(
            (field: any) => field.name === "time_taken",
          );
          const numTokensField = currentSchema.fields.find(
            (field: any) => field.name === "num_tokens",
          );
          const hasWrongTimeTakenType =
            timeTakenField && timeTakenField.type !== "FLOAT";
          const hasWrongNumTokensType =
            numTokensField && numTokensField.type !== "FLOAT";

          if (hasDocsField) {
            logger.warn(
              "Table has old schema with docs field, recreating table",
              {
                dataset: datasetId,
                table: tableId,
              },
            ...
            await table.delete();
            logger.info("Deleted table with old schema (docs field)", {
              dataset: datasetId,
              table: tableId,
            });
          } else if (hasWrongTimeTakenType || hasWrongNumTokensType) {
            logger.warn(
              "Table has wrong data types for numeric fields, recreating table",
              {
                dataset: datasetId,
                table: tableId,
            ...
            await table.delete();
            logger.info(
              "Deleted table with wrong schema (numeric field types)",
        ...
        try {
          await table.delete();
          logger.info("Deleted problematic table", {
            dataset: datasetId,
            table: tableId,
          });
        } catch (deleteError) {
          logger.error("Error deleting problematic table", {
            error: deleteError,
          });
        }
      }
      ...
      while (i++ <= 10 && !done) {
        try {
          ...
          if (error.message && error.message.includes("schema")) {
            jobLogger.error("Schema error detected, stopping retries", {
              error: error.message,
            });
            break;
          }
